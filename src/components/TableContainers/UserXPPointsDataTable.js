...
import { fetchMoodsRequest, fetchUsersRequest, fetchUserXPPointsRequest, getClassifications, getEvents } from "store/actions";
import { useDispatch } from "react-redux";
import { DebouncedInput } from "helpers/common_helper";
import Pagination from "components/Common/Pagination";
import { fetchUserTokensRequest } from "store/UserTokens/actions";
import axiosInstance from "pages/Utility/axiosInstance";


// Global Filter (Debounced Input)

const UserXPPointsDataTable = ({
    columns,
    data,
    tableClass,
    isPagination = true,
    ...
    useEffect(() => {
        const fetchCustomers = async () => {
            try {
                const { data } = await axiosInstance.get("V1/customers/dropdown/all");  
                if (data) {
    ...
    useEffect(() => {
        ...
        dispatch(fetchUserXPPointsRequest(params));
    }, [selectedSortData, pageIndex, searchString,selectedCustomer])
            ...
            <div className="d-flex justify-content-between gap-2 mb-2">
                <div className="d-flex justify-content-between gap-2 align-items-center">
                    <Select
                        ...
                        isLoading={loading}
                        options={customers}
                        value={customers?.find((c) => c.value === selectedCustomer) || null}
...
export default UserXPPointsDataTable;
