...
import SubscriptionDataTable from "components/TableContainers/SubscriptionDataTable";
import CreateSubscription from "./CreateSubscription";

const SubscriptionTable = ({ subscriptions, loading, totalrows }) => {
  ...
  const hasEditPermission = permissions?.some(
    (item) => item?.permission_name === "Subscription Edit"
  );
  const hasDeletePermission = permissions?.some(
    (item) => item?.permission_name === "Subscription Delete"
  );
  ...
  const columns = useMemo(
    () => [
      ...
      { 
        ...
        cell: ({ row }) => (
            <span>{row.original.plan?.plan_name}</span>
          ),
     },
      {
        header: "Created At",
